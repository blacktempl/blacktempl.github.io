...
* {{ site.data.conduits.Repeat_Decree }} — способность {{ site.data.covenant.Elysian_Decree }} устанавливает вторую руну через 1 секунду, которая наносит 15-24% урона. Усиливает и без того сильную способность ковенанта, но нужен высокий илвл кондуита.
* {{ site.data.conduits.Growing_Inferno }} — урон способности {{ site.data.spell.Immolation_Aura }} дополнительно увеличивается на 10-16% каждый раз, когда она наносит урон. Очень сильно увеличивает урон от {{ site.data.spell.Immolation_Aura }}, используем всегда.
* {{ site.data.conduits.Demons_Touch }} —  {{ site.data.spell.Chaos_Strike }} с вероятностью 5-8% сработает еще раз. Очень сильный кондуит для СТ боев.


<u>Проводники выносливости</u>

* {{ site.data.conduits.Viscous_Ink }} — {{ site.data.spell.Demonic_WardsHDH }} снижает получаемый вами урон от магии дополнительно на 6-13%. Очень сильный проводник, берем когда в бою присутствует магический урон.
* {{ site.data.conduits.Fel_Defender }} — время восстановления вашей способности {{ site.data.spell.Blur }} уменьшается на 5-11 сек. Выбор по умолчанию в боях без магии.

<u>Проводники точности</u>

* {{ site.data.conduits.Felfire_Haste }} — ваша скорость передвижения повышается на 5-11% после использования способности {{ site.data.spell.Fel_Rush }}. Дает дополнительную мобильность и без того мобильному классу. Все же является единственным универсальным выбором проводника в эту ячейку.
* {{ site.data.conduits.Ravenous_Consumption }} — способность {{ site.data.spell.Consume_Magic }} получает 15-21% шанс рассеять дополнительный эффект.
...
> Указания по выбору слота для легендарных предметов сделаны "с точки зрения минмакса" для персонажа, который закрывает мифик рейды и имеет высокий пвп рейтинг. Другими словами - для персонажа имеющего доступ к любому шмоту (виртуально). Если перед вами стоит вопрос "куда мне крафтить легу", то вам стоит посмотреть на те предметы что у вас есть. К примеру, вы хотите скрафтить  :inv_jewelry:  **Дар созерцателя тьмы **. Эта лега крафтится в слот шеи или пояса. У вас есть 226й пояс с бис статами, а вот шея с бис статами для дх добывается с ПвП. Если взятие 2100 в пвп вам "не светит", то в вашем случае вам имеет смысл сделать Дар созерцателя тьмы в слот шеи.
...
> Оба билда хорошо себя показывают в рейде, билд с {{ site.data.legendary.Burning_Wound }} более стабилен и не зависит от проков.
...
> Билд с {{ site.data.legendary.Burning_Wound }}  даст стабильный клив урон на протяжении всего боя, билд с {{ site.data.legendary.Darkglare_Boon }} при удачных проках может выдать выше урон, но опять же игра в рулетку.
...
> Билд через {{ site.data.legendary.Burning_Wound }} обеспечит нам стабильный урон и избавит от рандома, Билды с {{ site.data.legendary.Chaos_Theory }} вносит в игру рандом но при удачных обстоятельствах могут выдать выше урон.
...
> Если вы играете с ковенантом <img src="{{ site.url }}/assets/img/guide/havoc/nightfae.png" width="22" height="22"> **Ночной народец** и у вас есть возможность сдавать свои КД в адов,  вы можете взять в качестве медиума **Корейна** и билд с {{ site.data.legendary.Collective_Anguish }}. У вас будет сильный бурcт урон в каждый новый пак адов, а также вы сможете продлевать баф крита за счет "опоздавших" адов которых еще не успели свести. Но если у вас нет такой возможности берем в качестве медиума Нию.
...
> На <img src="{{ site.url }}/assets/img/guide/havoc/zimox.png" width="24" height="24"> Изобретателе Зи'моксе в билде через ФБ+ДБ (**3321221**) можно взять вместо {{ site.data.talent.Unbound_Chaos }}, {{ site.data.talent.Trail_of_Ruin }} если в рейде ловушки чистятся не регулярно/плохо, или если вы банально не уверены в том, что не сможете в них не попадать. Так же, если вас назначают носить семки (ну мало ли), стоит следить за таймером семян и за вашим КД {{ site.data.talent.Fel_Rush }}.
...
> Если вы на этом боссе плохо следите за таймерами и не успеваете быть в назначенном месте под вальс или часто ловите клив от Ставроса из-за рывков, то в таком случае {{ site.data.talent.Unbound_Chaos }} в рекомендуемом билде можно заменить на {{ site.data.talent.Glaive_Tempest }}, а в альтернативном билде на {{ site.data.talent.Trail_of_Ruin }}.
...
> Вместо {{ site.data.talent.Unbound_Chaos }}, можно взять {{ site.data.talent.Glaive_Tempest }}, но это менее профитно
...
> **Внимание** Этот билд стоит брать если вы хотите накрутить и только если ваш рейд может себе позволить взрывать ВСЕХ аддов на фазах за пару секунд. Если вы себе этого позволить не можете, то этот билд контрпродуктивен т.к. он будет приводить к вайпам из-за того, что все адды будут на лоу хп и вас будет просто тупо сдувать с платформы.
...
> Корейн или Ниа. Оба показывают примерно одинаковую пользу. Корейн очень сильно апает урон ДХ на первых двух фазах, однако Ниа больше поможет с уроном на ласт фазе. По этому, Нию лучше взять на освоение, т.к. как правило вас ждут вайпы где у босса ХП остается "на один чих". На фарме можно Нию поменять на Корейна.
